...
public class BetterPoseEstimator {
  ...
  public void resetTranslation(Translation2d translation) {
    m_odometry.resetOdometry(new Pose2d(translation, m_odometry.getPose().getRotation()));
    ;
    m_odometryPoseBuffer.clear();
    m_visionUpdates.clear();
    m_poseEstimate = m_odometry.getPose();
  ...
  public void resetRotation(Rotation2d rotation) {
    m_odometry.resetOdometry(new Pose2d(m_odometry.getPose().getTranslation(), rotation));
    m_odometryPoseBuffer.clear();
    m_visionUpdates.clear();
    m_poseEstimate = m_odometry.getPose();
  }
   ...
   * BetterPoseEstimator#update} every loop.
   *
   ...
   * BetterPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
  ...
  public Pose2d update() {
    return updateWithTime(MathSharedStore.getTimestamp());
  }
  ...
  public Pose2d updateWithTime(double currentTimeSeconds) {
    m_odometry.update();
    var odometryEstimate = m_odometry.getPose();
