...
/**
 * Simple Motor that utilizes a {@link DoubleConsumer} for setting power and provides no feedback. Can
 * work with {@link TalonFX}'s and {@link SparkBase}'s. Can be closed with the specified {@link Runnable}.
 */
public class SimpleMotor {
   ...
   */
  public SimpleMotor(
      DoubleConsumer set, Runnable close) {
    this.set = set;
    this.close = close;
  }
   ...
   * @param config : {@lzink TalonFXConfiguration} to apply to the motor.
   */
